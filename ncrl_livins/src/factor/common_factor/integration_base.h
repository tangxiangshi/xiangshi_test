...
class IntegrationBase
{
  public:
                            ...
                            Eigen::Vector3d &result_linearized_ba, Eigen::Vector3d &result_linearized_bg, bool update_jacobian)
    {
        ...
        // midpoint integration derivative each delta state to construct a linearize function of
        if(update_jacobian)
        {
            Vector3d w_x = 0.5 * (_gyr_0 + _gyr_1) - linearized_bg;
            Vector3d a_0_x = _acc_0 - linearized_ba; // remove the bias's acceleration
    ...
    void propagate(double _dt, const Eigen::Vector3d &_acc_1, const Eigen::Vector3d &_gyr_1)
    {
        ...
        acc_1 = _acc_1;
        gyr_1 = _gyr_1;
        Vector3d result_delta_p; // translation
        Quaterniond result_delta_q; // rotation
        Vector3d result_delta_v; // velocity
        Vector3d result_linearized_ba; // linearize acc bias
        Vector3d result_linearized_bg; // linearize gyr bias
    ...
    // input two combine data's state
                                          ...
                                          const Eigen::Vector3d &Pj, const Eigen::Quaterniond &Qj, const Eigen::Vector3d &Vj, const Eigen::Vector3d &Baj, const Eigen::Vector3d &Bgj)
    {
        ...
        Eigen::Matrix3d dp_dba = jacobian.block<3, 3>(O_P, O_BA); // delta velocity derivative of delta acc bias
        ...
        Eigen::Vector3d dba = Bai - linearized_ba; // delta acc_bias
        Eigen::Vector3d dbg = Bgi - linearized_bg; // delta gyro_bias

        Eigen::Quaterniond corrected_delta_q = delta_q * Utility::deltaQ(dq_dbg * dbg); // bias
...
//        std::cout << "gravity : " << G << std::endl;
        ...
        // in one frame
        // ***** state on world frame ***** then trans to i frame
    ...
    const Eigen::Vector3d linearized_acc, linearized_gyr; // only store when Integration is declared
