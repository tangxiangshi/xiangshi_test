...
public:
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        residual = sqrt_info * nu.cross(last_point_b - point_w);
        ...
        if (jacobians)
        {
            ...
            if (jacobians[0])
            {
                ...
                jaco_i.rightCols<3>() = Utility::skewSymmetric(nu) * Rj * Utility::skewSymmetric(point_j);
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians)
        {
            ...
            if (jacobians[0])
            {
                ...
                jaco_j.rightCols<3>() = -plane_unit_norm.transpose() * Rj * Utility::skewSymmetric(point_j);
