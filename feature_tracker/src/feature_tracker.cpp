...
void FeatureTracker::setMask()
{
  ...
  // prefer to keep features that are tracked for long time
    ...
    for (unsigned int i = 0; i < forw_pts.size(); i++)
        cnt_pts_id.push_back(make_pair(track_cnt[i], std::make_pair(forw_pts[i], ids[i])));
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
  ...
  if(PUB_THIS_FRAME)
  {
    ...
    if(n_max_cnt > 0)
    {
      ...
      // input_image, output_corners, max nums of corners, quality level, min_euler_distance, mask
